...
const HELP = `
Usage: swooshrouter xai video --prompt "..." [options]

Generate a Grok Imagine video via your local SwooshRouter gateway
...
Options:
  ...
  --api-key <key>         SwooshRouter API key (or env SWOOSHROUTER_API_KEY)
...
function parseArgs(argv) {
  const opts = {
    ...
    port: DEFAULT_PORT,
    host: DEFAULT_HOST,
    apiKey: process.env.SWOOSHROUTER_API_KEY || null,
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-swooshrouter-connection-id"] || null;
