...
const INSTALL_MANIFEST = ".swooshrouter-install.json";
...
function defaultDataDir() {
  ...
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), ".swooshrouter")
    : path.join(os.homedir(), ".swooshrouter");
}

function candidateInstallDirs() {
  const dirs = [];
  if (process.env.SWOOSH_ROUTER_DIR) dirs.push(process.env.SWOOSH_ROUTER_DIR);
  if (process.platform === "win32") {
    dirs.push(path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "SwooshRouter"));
  } else {
    dirs.push(path.join(os.homedir(), ".local", "share", "swooshrouter"));
    dirs.push("/opt/swooshrouter");
  }
  return dirs;
}
...
function inspectDirectory(dir, manifest = null) {
  ...
  const binaryFile = fileExists(path.join(resolved, binaryName)) ? path.join(resolved, binaryName) : null;
...
function discoverInstall({ dir, cliDirectory } = {}) {
  const explicit = dir || process.env.SWOOSH_ROUTER_DIR;
  ...
  const siblingCandidates = [
    path.join(cliDirectory || path.join(__dirname, "..", "..", ".."), binaryName),
