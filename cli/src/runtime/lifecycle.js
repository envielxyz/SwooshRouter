...
function spawnNative(installation, { host, port, showLog = false, detached = false } = {}) {
  ...
  if (!command) {
    throw new Error(isBinary
      ? "Compiled Swoosh Router binary was not found"
...
function dataDirectory(installation) {
  ...
  return path.join(process.platform === "win32"
    ...
    : os.homedir(), ".swooshrouter");
}

function pidFile(installation) {
  return path.join(dataDirectory(installation), "runtime", "swooshrouter.pid");
}
...
function ownsProcess(installation, record) {
  ...
  const installDir = path.resolve(record.installDir || installation.installDir).toLowerCase();
  const executable = record.executable ? path.resolve(record.executable).toLowerCase() : "";
  return command.includes(installDir) || (executable && command.includes(executable));
}

function stopNative(installation) {
  const record = readPidRecord(installation);
  if (!record || !processAlive(record.pid)) {
    if (record) removePid(installation);
...
function waitForProcessOrReady(child, port, host) {
  return new Promise((resolve, reject) => {
    ...
    const onClose = (code, signal) => finish(reject, new Error(`Swoosh Router exited before readiness (code=${code ?? "unknown"}, signal=${signal || "none"})`));
    child.once("error", onError);
    child.once("close", onClose);
    waitForReady(port, { host }).then((ready) => {
      if (ready) finish(resolve, true);
      else finish(reject, new Error(`Swoosh Router did not become ready on port ${port}`));
    });
  });
}
...
async function start(installation, { background = false, showLog = false, port, host } = {}) {
  ...
  if (status(installation).running) throw new Error("Swoosh Router is already running; stop it before starting again");
  ...
  if (installation.mode === "docker") {
    ...
    if (background) {
      ...
      if (!await waitForReady(targetPort, { host: targetHost })) {
        throw new Error(`Swoosh Router did not become ready on port ${targetPort}`);
      }
    } else {
  ...
  if (!child.pid) throw new Error("Swoosh Router process did not start");
  ...
  try {
    ...
    if (!ready) throw new Error(`Swoosh Router did not become ready on port ${targetPort}`);
  } catch (error) {
...
function status(installation) {
  ...
  const record = readPidRecord(installation);
  return { running: ownsProcess(installation, record), pid: record?.pid || null };
}
