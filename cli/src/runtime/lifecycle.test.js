...
describe("runtime lifecycle helpers", () => {
  ...
  test("parses Docker running and stopped status", () => {
    ...
    expect(lifecycle.parseDockerStatus("swooshrouter   Up 2 minutes").running).toBe(true);
  });
  ...
  test("rejects legacy PID records as unmanaged", () => {
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-lifecycle-"));
  ...
  test("records a compiled binary in managed PID metadata", () => {
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-lifecycle-"));
    const installation = { installDir: process.cwd(), dataDir, mode: "binary", binaryFile: path.join(process.cwd(), "swooshrouter") };
    const file = lifecycle.pidFile(installation);
    try {
      lifecycle.writePid?.(installation, process.pid);
      if (fs.existsSync(file)) expect(JSON.parse(fs.readFileSync(file, "utf8")).executable).toContain("swooshrouter");
    } finally {
      fs.rmSync(dataDir, { recursive: true, force: true });
    }
  });
  ...
  test("defaults native CLI launches to production without requiring an env file", () => {
    const installation = {
      ...
      dataDir: path.join(os.tmpdir(), "swoosh-cli-production-env"),
      env: {},
    };
