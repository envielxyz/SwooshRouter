...
describe("runtime discovery", () => {
  test("reads native .env and manifest metadata", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-discover-"));
    try {
      fs.mkdirSync(path.join(root, "src"));
      fs.writeFileSync(path.join(root, "src", "server.ts"), "export {};\n");
      fs.writeFileSync(path.join(root, ".env"), 'PORT="8080"\nJWT_SECRET="value"\n');
      fs.writeFileSync(path.join(root, ".swooshrouter-install.json"), JSON.stringify({ mode: "native", port: 8080, dataDir: path.join(root, "data") }));
  ...
  test("explicit missing directory is not discovered", () => {
    expect(discoverInstall({ dir: path.join(os.tmpdir(), "not-a-swoosh-install", String(Date.now())) })).toBe(null);
  });

  test("rejects legacy bundled manifests with migration guidance", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-bundled-"));
    try {
      fs.writeFileSync(path.join(root, ".swooshrouter-install.json"), JSON.stringify({ mode: "bundled" }));
  ...
  test("discovers compiled binaries without treating them as npm bundles", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-binary-"));
    const binaryName = process.platform === "win32" ? "swooshrouter.exe" : "swooshrouter";
    try {
  ...
  test("discovers the repository root when cli directory is supplied", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-source-"));
    try {
      fs.mkdirSync(path.join(root, "src"));
      fs.writeFileSync(path.join(root, "src", "server.ts"), "export {};\n");
  ...
  test("discovers a Docker checkout", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-docker-"));
    try {
  ...
  test("reads single-quoted dotenv values", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-dotenv-"));
    try {
  ...
  test("uses a registered data directory for a source checkout", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "swoosh-manifest-"));
    try {
      fs.mkdirSync(path.join(root, "src"));
      fs.writeFileSync(path.join(root, "src", "server.ts"), "export {};\n");
      const dataDir = path.join(root, "data");
      fs.writeFileSync(path.join(root, ".swooshrouter-install.json"), JSON.stringify({ mode: "native", dataDir }));
