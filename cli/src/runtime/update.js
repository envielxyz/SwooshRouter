...
function registryUrl() {
  return String(process.env.SWOOSHROUTER_NPM_REGISTRY || "https://registry.npmjs.org").replace(/\/+$/, "");
}

async function checkForUpdate(fetchImpl = globalThis.fetch) {
  const currentVersion = String(pkg.version || "0.0.0");
  const packageName = process.env.SWOOSHROUTER_PACKAGE_NAME || pkg.name || "swooshrouter";
...
function packageManager() {
  const customCommand = String(process.env.SWOOSHROUTER_PACKAGE_MANAGER_PATH || "").trim();
  if (customCommand) return { command: customCommand, args: ["install", "--global"] };

  const preferred = String(process.env.SWOOSHROUTER_PACKAGE_MANAGER || "").trim().toLowerCase();
...
function installPackageVersion(version) {
  ...
  const spec = `${process.env.SWOOSHROUTER_PACKAGE_NAME || pkg.name || "swooshrouter"}@${version}`;
  return new Promise((resolve, reject) => {
    const child = spawn(manager.command, [...manager.args, spec], {
      stdio: "inherit",
      windowsHide: true,
    });
