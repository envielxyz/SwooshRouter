    ...
    try {
      await onSubmit({
        ...
        bulkKeys: validBulkRows.map(({ name, apiKey }, index) => {
          ...
          return {
            name: explicitName || `swoosh-account${index + 1}`,
            apiKey: apiKey.trim(),
    ...
    function completeCallback(payload: OAuthCallbackPayload) {
      ...
      void (async () => {
        try {
          ...
          oauthCompletionRef.current = false;
          setOauthBootError("OAuth exchange failed. Paste the callback code and click Connect.");
        } finally {
          setLoading(false);
        }
      })();
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await onSubmit({
        ...
        oauthCode: isOAuthFlow ? oauthCode.trim() || undefined : undefined,
        oauthState: isOAuthFlow ? oauthState || undefined : undefined,
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="Auto: swoosh-account"
              value={name}
