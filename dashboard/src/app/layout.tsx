...
const SwooshChat = lazy(() => import("@/pages/SwooshChat"));

export function ProtectedLayout() {
  const location = useLocation();
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const isSwooshChatRoute = location.pathname === "/dashboard/swoosh-chat";
  const [chatMounted, setChatMounted] = useState(isSwooshChatRoute);

  useEffect(() => {
    if (isSwooshChatRoute) setChatMounted(true);
  }, [isSwooshChatRoute]);
      ...
      <div className="flex h-full min-h-0 w-full min-w-0">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          >
            ...
            {chatMounted ? (
              <div className={cn("min-h-0 flex-1", isSwooshChatRoute ? "flex" : "hidden")}>
