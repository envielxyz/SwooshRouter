...
import {
  ...
  type Attachment,
  type ChatMessage,
  SWOOSH_CHAT_WORKSPACE_STORAGE,
  parseSwooshChatWorkspace,
  reconcileSwooshChatModel,
  serializeSwooshChatWorkspace,
} from "@/lib/chatStudio";
import {
  SWOOSH_AGENT_TOOLS,
  agentToolLabel,
...
const AgentActivityView = memo(function AgentActivityView({ activities }: { activities?: AgentToolActivity[] }) {
  ...
  return (
    <div className="mb-2 space-y-1.5" aria-label="Swoosh Router Assistant activity">
          ...
          <div key={activity.id} className="rounded-lg border border-border/70 bg-surface/45 px-2.5 py-1.5 text-[11px]">
            ...
            {imageSources.length ? (
              <div className="mt-2 flex flex-wrap gap-2">
                {imageSources.map((src, index) => <img key={`${activity.id}-${index}`} src={src} alt="Generated by Swoosh Router Assistant" className="max-h-64 max-w-full rounded-lg border border-border object-contain" />)}
              </div>
            ) : null}
          </div>
        );
...
export default function SwooshChat() {
  const navigate = useNavigate();
  const [workspace] = useState(() => {
    try {
      return parseSwooshChatWorkspace(window.localStorage.getItem(SWOOSH_CHAT_WORKSPACE_STORAGE));
    } catch {
      return null;
    }
  });
  ...
  useEffect(() => {
    ...
    const nextModel = reconcileSwooshChatModel(model || storedModel, modelsQ.data);
    setModel(nextModel);
  }, [modelsQ.data, model]);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      const raw = serializeSwooshChatWorkspace({ model, systemPrompt, messages });
      if (raw) {
        try { window.localStorage.setItem(SWOOSH_CHAT_WORKSPACE_STORAGE, raw); } catch {                                     }
      }
  ...
  function clearConversation() {
    ...
    try { window.localStorage.removeItem(SWOOSH_CHAT_WORKSPACE_STORAGE); } catch {                               }
    setMessages([]);
    setInlineError("");
  ...
  async function streamReply(
    ...
    if (enableTools) {
      requestBody.tools = SWOOSH_AGENT_TOOLS;
    ...
    for (let round = 0; round < maxToolRounds; round += 1) {
      ...
      for (const call of toolCalls) {
        ...
        try {
          ...
          toolResult = await executeSwooshTool(call.function.name, args, { apiKey, approve: requestAgentApproval });
    ...
    <div className="flex h-full min-h-0 min-w-0 w-full max-w-full overflow-x-hidden pb-2">
                  ...
                  <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mb-3 rounded-xl border border-amber-300/30 bg-amber-300/[0.06] p-3" role="dialog" aria-label="Approve JavaScript execution">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[11px] leading-5 text-muted-foreground">Swoosh Router Assistant requested a diagnostic script. It runs locally with a reduced environment and a 10-second timeout.</p>
                ...
                {messages.length === 0 ? (
                  <div className="flex h-full min-h-[30vh] flex-col items-center justify-center gap-2 px-6 text-center">
                    <span className="grid size-12 place-items-center rounded-xl border border-border bg-surface text-muted-foreground">
                      <img src="/logo/swooshchat.svg" alt="" width="24" height="24" className="size-6 object-contain" />
                    </span>
                    <div className="space-y-1"><h2 className="text-sm font-semibold">Swoosh</h2><p className="mx-auto max-w-md text-xs leading-5 text-muted-foreground">Pick a model, drop a prompt.</p></div>
                  </div>
                ) : (
                  <AnimatePresence initial={false}>
                    {messages.map((m) => {
      ...
      <Dialog open={keyGate === "blocked"} onOpenChange={(o) => { if (!o) { setKeyGate("ready"); navigate(API_KEYS_ROUTE); } }}>
        <DialogContent>
          ...
          <DialogDescription>The gateway is set to require an API key for chat requests. Create one in the API Keys menu — it takes a few seconds and Swoosh Chat will use it automatically.</DialogDescription>
          </DialogHeader>
