...
export const SWOOSH_AGENT_TOOLS = [
  {
    type: "function",
    function: {
      name: "router_overview",
      description: "Inspect the local Swoosh Router status. The result explicitly separates totalProviders (the unique union of built-in registry providers and custom provider nodes, including zero-connection providers), registeredProviderTypes, providerNodes, configuredProviders, activeProviders, inactiveOnlyProviders, connections, API keys, proxy pools, combos, admission load, and safe settings. When asked for the total number of providers, use totalProviders, not activeProviders or connectedProviders.",
  ...
  },
  {
    ...
    function: {
      name: "router_models",
      description: "List models currently exposed by Swoosh Router, including provider and known capabilities such as vision, tools, reasoning, and image output.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      name: "generate_image",
      description: "Generate an image through an enabled Swoosh Router media provider. Use only when the user asks for an image and report clearly if media generation is not configured.",
      parameters: {
        type: "object",
        properties: {
...
export async function executeSwooshTool(
  name: string,
