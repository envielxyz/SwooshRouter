...
describe("cli config formatter", () => {
  it("masks secrets by default and normalizes OpenCode models", () => {
    ...
    expect(parsed.provider.swooshrouter.options.baseURL).toBe("http://127.0.0.1:14045/v1");
    expect(parsed.provider.swooshrouter.models["clinepass/glm-5.2"]).toBeTruthy();
    expect(parsed.model).toBe("swooshrouter/openai/gpt-5");
    expect(parsed.agent.explorer.model).toBe("swooshrouter/clinepass/glm-5.2");
  ...
  it("allows endpoint config without inventing a model", () => {
    ...
    expect(result.content).toContain('model_provider = "swooshrouter"');
