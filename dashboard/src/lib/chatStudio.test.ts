...
describe("Swoosh Chat workspace persistence", () => {
  test("round-trips messages and system prompt", () => {
    const raw = serializeSwooshChatWorkspace({ model: "openai/gpt", systemPrompt: "Be concise", messages: [um("hello"), am("hi")] });
    expect(parseSwooshChatWorkspace(raw)).toMatchObject({ model: "openai/gpt", systemPrompt: "Be concise", messages: [{ content: "hello" }, { content: "hi" }] });
  });

  test("rejects malformed or unsupported workspace data", () => {
    expect(parseSwooshChatWorkspace("not json")).toBeNull();
    expect(parseSwooshChatWorkspace(JSON.stringify({ schemaVersion: 99, messages: [] }))).toBeNull();
  });
  ...
  test("filters malformed messages and reconciles unavailable models", () => {
    ...
    expect(parseSwooshChatWorkspace(raw)?.messages).toHaveLength(1);
    expect(reconcileSwooshChatModel("gone", [{ id: "keep", name: "Keep", provider: "test" }])).toBe("keep");
  });
  ...
  test("normalizes persisted streaming assistants to interrupted errors without dropping stream state", () => {
    ...
    const parsed = parseSwooshChatWorkspace(raw, 10_000);
  ...
  test("drops legacy tool turns from restored workspaces", () => {
    ...
    expect(parseSwooshChatWorkspace(raw)?.messages).toEqual([
...
describe("A7.1 Model Studio engine — misc", () => {
  test("adds authoritative runtime model context to the system prompt", () => {
    const prompt = buildModelAwareSystemPrompt("You are Swoosh Router.", "custom-provider/zai-org/GLM-5.2");
