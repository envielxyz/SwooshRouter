...
export function buildModelAwareSystemPrompt(basePrompt: string, modelId: string): string {
  ...
  return [
    ...
    "",
    "Identity rules:",
    "- You are Swoosh Router Assistant, the internal agent and operator of the Swoosh Router platform; do not present yourself as the underlying provider or model.",
...
export const SWOOSH_CHAT_WORKSPACE_VERSION = 1;
export const SWOOSH_CHAT_WORKSPACE_STORAGE = "swoosh-chat.workspace.v1";
...
export type SwooshChatWorkspace = {
...
export function parseSwooshChatWorkspace(raw: string | null, now = Date.now()): SwooshChatWorkspace | null {
  if (!raw) return null;
  try {
    const value: unknown = JSON.parse(raw);
    if (!isRecord(value) || value.schemaVersion !== SWOOSH_CHAT_WORKSPACE_VERSION) return null;
    const messages = Array.isArray(value.messages)
      ? value.messages.map((item) => parseMessage(item, now)).filter((item): item is ChatMessage => !!item).slice(-200)
      : [];
    return {
      schemaVersion: SWOOSH_CHAT_WORKSPACE_VERSION,
...
export function serializeSwooshChatWorkspace(workspace: Omit<SwooshChatWorkspace, "schemaVersion">): string | null {
  try {

    const messages = workspace.messages.map(({ attachments: _attachments, toolActivity: _toolActivity, ...message }) => message);
    return JSON.stringify({ schemaVersion: SWOOSH_CHAT_WORKSPACE_VERSION, ...workspace, messages });
  } catch {
    return null;
  }
}

export function reconcileSwooshChatModel(savedModel: string, models: ModelInfo[]): string {
