...
describe("runtime environment", () => {
  ...
  test("loads bounded history and upstream retry overrides", () => {
    const config = loadEnv({
      ...
      SWOOSH_USAGE_RETENTION_DAYS: "30",
      SWOOSH_USAGE_HISTORY_MAX_ROWS: "1200000",
      SWOOSH_UPSTREAM_MAX_ATTEMPTS: "4",
    });
  ...
  test("derives an SSE-friendly queue wait from the selected resource profile", () => {
    const config = loadEnv({
      ...
      SWOOSH_PERFORMANCE_PROFILE: "low-memory",
    });
  ...
  test("accepts complete production configuration", () => {
    const config = loadEnv({
      ...
      DATA_DIR: "./data",
      SWOOSH_METRICS_TOKEN: "metrics-secret",
      SWOOSH_METRICS_LOCAL: "0",
    });
