...
export function loadEnv(source: Record<string, string | undefined> = process.env): EnvConfig {
  ...
  const metricsToken = source.SWOOSH_METRICS_TOKEN?.trim() || "";
  const resource = deriveResourceProfile(undefined, source.SWOOSH_PERFORMANCE_PROFILE);
  const routerMaxConcurrent = parsePositiveInt(source.SWOOSH_ROUTER_MAX_CONCURRENT, resource.maxConcurrent, 100_000);
  const routerInitialConcurrent = Math.min(
    routerMaxConcurrent,
    parsePositiveInt(source.SWOOSH_ROUTER_INITIAL_CONCURRENT, resource.initialConcurrent, 100_000),
  );
  const routerMinConcurrent = Math.min(
    routerInitialConcurrent,
    parsePositiveInt(source.SWOOSH_ROUTER_MIN_CONCURRENT, resource.minConcurrent, 100_000),
  );
  const routerBodyBudgetBytes = parsePositiveInt(
    source.SWOOSH_ROUTER_BODY_BUDGET_MB,
    resource.bodyBudgetMb,
    16 * 1024,
  ) * 1024 * 1024;
  const routerMemoryHighWaterBytes = parsePositiveInt(
    source.SWOOSH_ROUTER_MEMORY_HIGH_WATER_MB,
    0,
    1024 * 1024,
  ) * 1024 * 1024;
  const routerMemoryCriticalWaterBytes = parsePositiveInt(
    source.SWOOSH_ROUTER_MEMORY_CRITICAL_MB,
    0,
    1024 * 1024,
  ) * 1024 * 1024;
  ...
  return Object.freeze({
    ...
    metricsToken,
    metricsLocal: source.SWOOSH_METRICS_LOCAL !== "0",
    ...
    routerInitialConcurrent,
    routerMinConcurrent,
    routerMaxQueue: parsePositiveInt(source.SWOOSH_ROUTER_MAX_QUEUE, resource.maxQueue, 1_000_000),
    routerQueueWaitMs: parsePositiveInt(
      source.SWOOSH_ROUTER_QUEUE_WAIT_MS,
      resource.queueWaitMs,
      10 * 60 * 1000,
    ),
    routerBodyBudgetBytes,
    routerBodyReadTimeoutMs: parsePositiveInt(source.SWOOSH_ROUTER_BODY_TIMEOUT_MS, 30_000, 10 * 60 * 1000),
    routerMemoryHighWaterBytes,
    routerMemoryCriticalWaterBytes,
    usageHistoryRetentionDays: parsePositiveInt(source.SWOOSH_USAGE_RETENTION_DAYS, 7, 3650),
    usageHistoryMaxRows: parsePositiveInt(source.SWOOSH_USAGE_HISTORY_MAX_ROWS, 500_000, 100_000_000),
    upstreamMaxAttempts: parsePositiveInt(
      source.SWOOSH_UPSTREAM_MAX_ATTEMPTS,
