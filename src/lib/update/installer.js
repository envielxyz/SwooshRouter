...
function isDockerRuntime() {
  return process.env.SWOOSHROUTER_RUNTIME_MODE === "docker"
...
export function installPackageVersion(version) {
  ...
  const spec = `${pkg.name || "swooshrouter"}@${version}`;
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code) => {
      if (code === 0) {
        resolve({ version, packageName: pkg.name || "swooshrouter", manager: update.command, output });
      } else {
