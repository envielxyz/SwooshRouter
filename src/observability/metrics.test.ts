...
describe("metrics: recordHttpRequest helper", () => {
  test("record → inc http_requests_total + observe http_request_duration_ms", () => {
    ...
    expect(text, `http requests total`).toMatch(/swoosh_http_requests_total\{method="POST",route="\/api\/v1\/chat\/completions",status="200"\} 2/);
    expect(text, `http duration count`).toMatch(/swoosh_http_request_duration_ms_count\{method="POST",route="\/api\/v1\/chat\/completions"\} 2/);
  });
});
