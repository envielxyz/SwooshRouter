...
describe("tracing: outbound + response stamping", () => {
  ...
  test("stampResponseWithTrace menambah x-swooshrouter-trace-id + traceparent", () => {
    ...
    expect(resp.headers.get("x-swooshrouter-trace-id"), `trace-id header`).toBe(HEX32);
  ...
  test("stampResponseWithTrace no-op saat trace null", () => {
    ...
    expect(resp.headers.get("x-swooshrouter-trace-id"), `no header`).toBeNull();
  });
  ...
  test("stampResponseTrace ALS convenience stamps inside scope", () => {
    ...
    expect(stamped!.headers.get("x-swooshrouter-trace-id"), `ALS stamp traceId`).toBe(HEX32);
  });
});
