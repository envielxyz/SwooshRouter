...
describe("request boundary", () => {
  test("rewrites supported public aliases without losing query strings", () => {
    ...
    expect(request.headers.get("x-swooshrouter-rewritten")).toBe("/api/v1/models");
  });
  ...
  test("only normalizes read-only QUERY requests", async () => {
    ...
    expect(normalized.headers.get("x-swooshrouter-query-method")).toBe("QUERY");
  });
  ...
  test("trusts forwarding headers only from loopback peers", () => {
    ...
    expect(local.headers.get("x-swooshrouter-real-ip")).toBe("203.0.113.2");
    ...
    expect(remote.headers.get("x-swooshrouter-real-ip")).toBe("198.51.100.4");
  });
