export function stampClientIp(raw: Request, peerIp: string | null): void {
  ...
  raw.headers.delete("x-swooshrouter-real-ip");
  raw.headers.delete("x-forwarded-for");
  raw.headers.delete("x-swooshrouter-via-proxy");
  raw.headers.set("x-swooshrouter-real-ip", ip);
  if (viaProxy) raw.headers.set("x-swooshrouter-via-proxy", "1");
}
...
export function getRequestBodyLimitBytes(pathname: string, method = "GET"): number {
  ...
  if (path === "/api/settings/database") return 25 * MIB;
  return 1 * MIB;
}
...
export function normalizeQueryRequest(raw: Request): Request {
  ...
  headers.set("x-swooshrouter-query-method", "QUERY");
...
export function rewritePublicAlias(raw: Request): Request {
  ...
  const to = (dest: string) => {
    ...
    next.headers.set("x-swooshrouter-rewritten", dest);
    return next;
  };
