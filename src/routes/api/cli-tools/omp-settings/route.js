...
const SWOOSH_MATCH_HOSTS = [
  "127.0.0.1:14045",
  "localhost:14045",
  "localhost",
  "127.0.0.1",
];
const SWOOSH_URL_HOST_RE = /^(?:127\.0\.0\.1|localhost)(?::14045)?$/;

function isSwooshRouterBaseUrl(baseUrl) {
  if (typeof baseUrl !== "string" || !baseUrl) return false;
  try {
    const u = new URL(baseUrl);
    return (
      u.pathname.endsWith("/v1") &&
      (SWOOSH_URL_HOST_RE.test(u.host) || SWOOSH_MATCH_HOSTS.includes(u.host))
    );
  } catch {
    return false;
  }
...
const providerPointsAtRouter = (provider) => {
  ...
  return isSwooshRouterBaseUrl(provider.baseUrl);
};
...
export async function GET(request) {
  try {
    ...
    const provider = config?.providers?.swooshrouter ?? null;
    const hasSwooshRouter = providerPointsAtRouter(provider);

    return NextResponse.json({
      installed: true,
      config: config ? stringifyYAML(config) : null,
      hasSwooshRouter,
      connected: hasSwooshRouter,
...
export async function POST(request, { params }) {
  try {
    ...
    setNestedSection(parsed, "providers.swooshrouter", provider);
...
export async function DELETE(request) {
  try {
    ...
    deleteNestedSection(parsed, "providers.swooshrouter");
    ...
    return NextResponse.json({
      success: true,
      message: "Swoosh Router settings removed successfully",
    });
  } catch (error) {
