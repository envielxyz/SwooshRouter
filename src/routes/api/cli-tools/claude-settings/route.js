...
function hasSwooshRouterFromSettings(settings) {
  return isSwooshRouterBaseUrl(settings?.env?.ANTHROPIC_BASE_URL);
}
...
export async function GET() {
  try {
    ...
    const hasSwooshRouter = hasSwooshRouterFromSettings(settings);
    const claudeJson = await readClaudeJson();

    return NextResponse.json({
      installed: true,
      settings: settings,
      hasSwooshRouter: hasSwooshRouter,
