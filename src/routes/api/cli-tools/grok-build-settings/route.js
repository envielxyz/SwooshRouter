...
const hasSwooshRouterConfig = (settings) => Boolean(settings?.model?.base_url);
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      installed: true,
      settings,
      hasSwooshRouter: hasSwooshRouterConfig(settings),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "swooshrouter model slots removed from Grok Build",
    });
  } catch (error) {
