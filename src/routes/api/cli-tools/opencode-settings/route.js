...
const hasSwooshRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["swooshrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["swooshrouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasSwooshRouter: hasSwooshRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("swooshrouter/") ? config.model.replace(/^swooshrouter\//, "") : null,
...
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_swooshrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    if (!config.provider) config.provider = {};

    const existingProvider = config.provider["swooshrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["swooshrouter"] = existingProvider;

    const finalActive = activeModel || modelsArray[0];
    if (activeModel === "") {
      if (config.model?.startsWith("swooshrouter/")) delete config.model;
    } else if (finalActive) {
      config.model = `swooshrouter/${finalActive}`;
    }
    ...
    if (effectiveSubagentModel) {
      ...
      config.agent.explorer = {
        ...
        mode: "subagent",
        model: `swooshrouter/${effectiveSubagentModel}`,
      };
    } else if (config.agent?.explorer?.model?.startsWith("swooshrouter/")) {
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("swooshrouter/")) {
        config.model = "";
      }
    }
...
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["swooshrouter"]?.models) {
      delete config.provider["swooshrouter"].models[modelToRemove];

      if (Object.keys(config.provider["swooshrouter"].models).length === 0) {
        delete config.provider["swooshrouter"];
        if (config.model?.startsWith("swooshrouter/")) delete config.model;
      } else if (config.model === `swooshrouter/${modelToRemove}`) {

        const remainingModels = Object.keys(config.provider["swooshrouter"].models);
        config.model = `swooshrouter/${remainingModels[0]}`;
      }
    } else {

      if (config.provider) delete config.provider["swooshrouter"];
      if (config.model?.startsWith("swooshrouter/")) delete config.model;
    }

    if (config.agent?.explorer?.model?.startsWith("swooshrouter/")) {
      delete config.agent.explorer;
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Swoosh Router settings removed from OpenCode",
    });
  } catch (error) {
