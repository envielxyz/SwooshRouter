...
const CLI_TOKEN_SALT = "swooshrouter-cli-auth";
...
async function getInternalHeaders() {
  ...
  headers["x-swooshrouter-cli-token"] = await getConsistentMachineId(CLI_TOKEN_SALT);
  return headers;
}
...
export async function pingModelByKind(
  ...
  if (options.connectionId) {
    headers["x-swooshrouter-connection-id"] = options.connectionId;
  }
  ...
  if (kind !== "llm" && kind !== undefined && kind !== null) {
    return {
      ...
      latencyMs: 0,
      status: 404,
      error: `Model kind "${kind}" is not supported — Swoosh is LLM-pure`,
    };
  }
