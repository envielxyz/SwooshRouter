...
describe("Swoosh Chat web search tool", () => {
  ...
  test("uses the no-key DuckDuckGo fallback and returns bounded results", async () => {
    ...
    globalThis.fetch = (async (input) => {
      ...
      return jsonResponse({
        Heading: "Swoosh Router",
        AbstractText: "A local-first AI gateway.",
        AbstractURL: "https://example.com/swoosh",
        RelatedTopics: [
          { Text: "Documentation", FirstURL: "https://example.com/docs" },
          { Text: "Ignored duplicate", FirstURL: "https://example.com/swoosh" },
        ],
      });
    });

    const response = await POST(searchRequest({ query: "Swoosh Router", max_results: 2 }));
    const body = await response.json();
    expect(response.status).toBe(200);
    expect(new URL(requestedUrl).hostname).toBe("api.duckduckgo.com");
    expect(body).toMatchObject({ ok: true, query: "Swoosh Router", provider: "duckduckgo", answer: "A local-first AI gateway." });
