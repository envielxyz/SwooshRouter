...
app.get("/", (c) => c.json({ ok: true, service: "swoosh-router", docs: "/health" }));
...
async function cliToken() {
  ...
  try {
    CLI_TOKEN_CACHE.v = await getConsistentMachineId("swooshrouter-cli-auth");
  } catch {
    CLI_TOKEN_CACHE.v = "";
  }
...
app.get("/metrics", async (c) => {
  const presented = c.req.header("x-swooshrouter-cli-token") || "";
  const isLocal =
    (c.req.header("x-swooshrouter-real-ip") || "").startsWith("127.0.0.1") ||
    (c.req.header("x-swooshrouter-real-ip") || "") === "::1";
...
console.log(`[swoosh-router] listening on http://localhost:${server.port}`);
